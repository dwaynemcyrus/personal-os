...
export async function exportNotesZip(
  ...
  // Load note metadata and body from the canonical items row.
  onProgress?.('Loading notes…');
  const { data: notes, error } = await supabase
    .from('items')
    .select('id, title, filename, updated_at, content')
    .eq('type', 'note')
    .eq('is_trashed', false)
    .order('updated_at', { ascending: false });
  if (error) throw error;

  const noteList = (notes ?? []) as Pick<ItemRow, 'id' | 'title' | 'filename' | 'updated_at' | 'content'>[];
